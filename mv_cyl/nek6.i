...
# ciMode = 6 rotates the case by 0.5 radians and uses the
# NekRS geometry solver.
P_ROT = 0.5

pex0 = 1.0

# NekRS ciTestErrors() tolerance.
CI_EPS = 5.0E-2

# NekRS ciMode = 6 reference errors.
REF_VOL_ERROR     = 0.2465620E-06
REF_YPISTON_ERROR = 0.2465620E-05
REF_TEMP_ERROR    = 0.4929804E-06

# Additional Cardinal-only test.
...
[Functions]
  ...
  # Local piston coordinate along the cylinder axis.
  # Do not project this by cos(P_ROT), because the CI reference error
  # is defined for the axial piston coordinate.
  [ypex]
    type = ParsedFunction
    expression = '-0.5 * (1.0 + cos(${P_OMEGA} * t))'
  []
...
[Postprocessors]
  ...
  # Reconstruct the piston coordinate along the local cylinder axis.
  [ypiston]
  ...
  # Resolve the moving-boundary velocity along the rotated piston direction.
  [termV_x]
  ...
  # NekRS-style normalized regression errors:
  #
  # abs(current_error - expected_error) / expected_error
  ...
  # Cardinal-only check; not present in ciTestErrors().
  [termV_pass]
    type = ParsedPostprocessor
  ...
  # Available ciMode = 6 checks plus the Cardinal-only termV check.
  #
  # Not available through current Cardinal postprocessors:
  #   p0th
  #   dp0thdt
